    ...
    <div class="col-12">
        ...
        <div class="chats-topbar mb-30 position-relative">
            <div class="d-block d-md-flex justify-content-between">
                <div class="d-block">
                    <h6 class="mb-0">{{$Report->student->name}}</h6>
                </div>
        ...
        <div class="scrollbar max-h-600" tabindex="3" style="overflow-y: auto; outline: none;">
            <div class="chats">
                @forelse ($Messages as $Message)
                    <div
                        class="chat-wrapper {{ $Message->type == 'teacher' && Auth::guard('parent')->check() ? 'chat-me' : 'chat-other' }} clearfix">
                        <div class="chat-avatar">
                            <img class="img-fluid avatar-small"
                                src="images/{{ $Message->type == 'teacher' ? 'teacher-avatar.jpg' : 'parent-avatar.jpg' }}"
                                alt="">
                        </div>
                        <div
                            class="chat-body {{ $Message->type == 'teacher' && Auth::guard('parent')->check() ? ' text-white' : 'bg-dark text-white' }} p-3">
                            <p>{{ $Message->message }}</p>
                        </div>
                    </div>
                    <span
        ...
        <div class="chats mt-30">
            <div class="chat-wrapper mb-0 clearfix">
                <div class="chat-input">
                    ...
                    <textarea required id="send" name="message" class="form-control input-message scrollbar" placeholder="Type here...*"
...
@push('js')
    ...
    $.ajax({
        url: "{{ route('teacher.messages.store','teacher') }}",
        method: "POST",
        data: {
            _token: "{{ csrf_token() }}",
            message: message,
